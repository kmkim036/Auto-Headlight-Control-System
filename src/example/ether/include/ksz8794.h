...
#define SW_MAC_TABLE_ENTRIES		32
...
#define TOTAL_PORT_NUM				SW_PORT_NUM //YOON==4

#define KSZ8795_COUNTER_NUM			0x20
#define TOTAL_KSZ8795_COUNTER_NUM	(KSZ8795_COUNTER_NUM + 4)
#define SW_COUNTER_NUM				KSZ8795_COUNTER_NUM
#define TOTAL_SW_COUNTER_NUM		TOTAL_KSZ8795_COUNTER_NUM
//=====================================================================
#include "yInc.h"
...
//-- For use SPI2 (PB12)
#define nCS0_KSZ_1      {GPIO_SetBits(GPIOB, GPIO_Pin_12);}   //PA15=0 case : GPIOA->BSRRL |= GPIO_Pin_15;
#define nCS0_KSZ_0      {GPIO_ResetBits(GPIOB, GPIO_Pin_12);} //PA15=1 case : GPIOA->BSRRH |= GPIO_Pin_15;
#endif
...
#define SW_LINK_AUTO_AGING		(1 << 0) //SW_LINK_AUTO_AGING
//-Reg@0x03
#define REG_SW_CTRL_1			0x03

#define SW_PASS_ALL				(1 << 7)
#define SW_HUGE_PACKET			(1 << 6) //2K Bytes packet support
#define SW_TX_FLOW_CTRL_DISABLE	(1 << 5) //802.1x
#define SW_RX_FLOW_CTRL_DISABLE	(1 << 4)//802.1x
...
#define SW_TAIL_TAG_ENABLE		(1 << 1) //Different from other chips
...
#define SW_MIRROR_RX_TX   (1 << 8) //Added by YOON :TBC
...
#define PORT_STATUS_LINK_GOOD      (1 << 5) //YOON ?
...
#define   SW_INVALID_VID_2_PORT3 (1 << 10)
/* forwarding invalid VID frames to port 2 */
#define   SW_INVALID_VID_2_PORT2 (1 << 9)
/* forwarding invalid VID frames to port 1 */
#define   SW_INVALID_VID_2_PORT1 (1 << 8)
/* enable insert vlan tag when untagged pkts from port 3 to port 2 */
#define   SW_INS_TAG_2_1         (1 << 5)
/* enable insert vlan tag when untagged pkts from port 3 to port 1 */
#define   SW_INS_TAG_2_0         (1 << 4)
/* enable insert vlan tag when untagged pkts from port 2 to port 3 */
#define   SW_INS_TAG_1_2         (1 << 3)
/* enable insert vlan tag when untagged pkts from port 2 to port 1 */
#define   SW_INS_TAG_1_0         (1 << 2)
/* enable insert vlan tag when untagged pkts from port 2 to port 3 */
#define   SW_INS_TAG_0_2         (1 << 1)
/* enable insert vlan tag when untagged pkts from port 1 to port 2 */
#define   SW_INS_TAG_0_1         (1 << 0)

#define SW_INS_TAG_1_PORT_2		SW_INS_TAG_0_1
#define SW_INS_TAG_1_PORT_3		SW_INS_TAG_0_2
#define SW_INS_TAG_2_PORT_1		SW_INS_TAG_1_0
#define SW_INS_TAG_2_PORT_3		SW_INS_TAG_1_2
#define SW_INS_TAG_3_PORT_1		SW_INS_TAG_2_0
#define SW_INS_TAG_3_PORT_2		SW_INS_TAG_2_1

//== macro ============================
#define PORT_CTRL_ADDR(port, addr)		(addr = REG_PORT_1_CTRL_0 + (port) * (REG_PORT_2_CTRL_0 - REG_PORT_1_CTRL_0))

//(3)=====(8794) Switch Engines MAC Addr Registers(used as the src addr or MAC Pause frames) ===============================
#define REG_SW_MAC_ADDR_0		0x68	//MACA[47..40]
#define REG_SW_MAC_ADDR_1		0x69	//MACA[39..32]
#define REG_SW_MAC_ADDR_2		0x6A	//MACA[31..24]
#define REG_SW_MAC_ADDR_3		0x6B	//MACA[23..16]
#define REG_SW_MAC_ADDR_4		0x6C	//MACA[15..8]
//#define REG_SW_MAC_ADDR_5		0x6D	//MACA[7..0] -- Not present. ??? See Table4-5. (override by each port?)
...
//0x6F = Indirect Address[7:0]

//- bit[7:5] = EEE/ACL/PME Indirect register function selection
#define TABLE_EXT_SELECT_S		5
                                                          //x00 = Indirect mode for table select in bits[3:2]
#define TABLE_EEE				(1 << TABLE_EXT_SELECT_S) //EEE
#define TABLE_ACL				(2 << TABLE_EXT_SELECT_S) //port based ACL
#define TABLE_PME				(4 << TABLE_EXT_SELECT_S) //PME
#define TABLE_LINK_MD			(5 << TABLE_EXT_SELECT_S) //LinkMD cable diag.
//- bit[4] = r1/w0
#define TABLE_READ				(1 << 4) //1=Read, 0=Write
//-bit[3:2] = Table Select or Indirect Address[11:10];
//-bit[1:0] = Indirect Address[9:8]
#define TABLE_SELECT_S			2
#define TABLE_STATIC_MAC		(0 << TABLE_SELECT_S) //Static MAC Addr Table
#define TABLE_VLAN				(1 << TABLE_SELECT_S) //VLAN table
#define TABLE_DYNAMIC_MAC		(2 << TABLE_SELECT_S) //Dynamic MAC Addr Table
#define TABLE_MIB				(3 << TABLE_SELECT_S) //MIB

#define REG_IND_CTRL_1			0x6F                  //Indirect Address[7:0]. When 0x6E==[000

#define TABLE_ENTRY_MASK		0x03FF
#define TABLE_EXT_ENTRY_MASK	0x0FFF
//- Register 0x6E/6F are used for tables, ..., or MIB counter, those are inditect data registers.
#define REG_IND_DATA_8			0x70	//[71:64]
#define REG_IND_DATA_7			0x71	//[63:56]
#define REG_IND_DATA_6			0x72	//[55:48]
#define REG_IND_DATA_5			0x73	//[47:40]
#define REG_IND_DATA_4			0x74	//[39:32]
#define REG_IND_DATA_3			0x75	//[31:24]
#define REG_IND_DATA_2			0x76	//[23:16]
#define REG_IND_DATA_1			0x77	//[15:8]
#define REG_IND_DATA_0			0x78	//[7:0]
...
//(5)== Advanced Control Registers(for Interrupts)
#define REG_INT_STATUS			0x7C //PME, Port3/2/1  Interrupt Status
#define REG_INT_ENABLE			0x7D //Interrupt Mask Registers (1=Enable)

#define INT_PME				(1 << 4) //PME Status : bit4

#define REG_ACL_INT_STATUS		0x7E //ACL Interrupt Status Reg
#define REG_ACL_INT_ENABLE		0x7F

//#define INT_PORT_5			(1 << 4)  //ACL_INT per port
//#define INT_PORT_4			(1 << 3)
#define INT_PORT_3			(1 << 2)
#define INT_PORT_2			(1 << 1)
#define INT_PORT_1			(1 << 0)
#define INT_PORT_ALL 		(INT_PORT_3 | INT_PORT_2 | INT_PORT_1)

//(6)== Advanced Control Registers(for Global Controls)
#define REG_SW_CTRL_12			0x80 //Global Control 12. (802.1p Tag)
#define REG_SW_CTRL_13			0x81 //Global Control 13. (802.1p Tag)

#define SW_802_1P_MASK		3
#define SW_802_1P_BASE		3
#define SW_802_1P_SHIFT		2

#define SW_802_1P_MAP_M			KS_PRIO_M
#define SW_802_1P_MAP_S			KS_PRIO_S

//(7) ==Advanced Control Registers(for ToS/DiffServ)
#define REG_SW_CTRL_14		0x82 //Global Control 14.
...
//(8) ==Advanced Control Registers
#define REG_SW_CTRL_15			0x83 //Global Control 15(Unkown unicast packet forward = disabled)
#define REG_SW_CTRL_16			0x84 //Global Control 16.(Unkown multicast packet forward = disabled)
#define REG_SW_CTRL_17			0x85 //Global Control 17.(Unkown VID packet forward = disabled)
#define REG_SW_CTRL_18			0x86 //Global Control 18. (Self-address filter enabled(0), unknown IP multicast packet forward(0),
...
//(9) ==Advanced Control Registers (Global Control 19. (Rate Limit, PVID..)
...
//(10) == Advanced Control Registers (ToS Prio Control Register 0~15)
...
//(11) == Advanced Control Registers (Global Control Reg 20) , Current Strength on MII
...
//(12) == Advanced Control Registers (Global Control Reg 21) - IPv6 MLD Snooping
#define REG_SW_CTRL_21			0xA4

#define SW_IPV6_MLD_OPTION		(1 << 3)
#define SW_IPV6_MLD_SNOOP		(1 << 2)

//(13) == Advanced Control Registers (Port n Control 12)
...
#define PORT_PASS_ALL			(1 << 6) //pass all frames
#define PORT_INS_TAG_FOR_PORT_5_S	3
#define PORT_INS_TAG_FOR_PORT_5		(1 << 3) //Inserat Source Port PVID for untagged frame to Top Prio Egress port.
#define PORT_INS_TAG_FOR_PORT_4		(1 << 2)
#define PORT_INS_TAG_FOR_PORT_3		(1 << 1)
#define PORT_INS_TAG_FOR_PORT_2		(1 << 0)//Inserat Source Port PVID for untagged frame to Lowest Prio Egress port.
//(14) == Advanced Control Registers (Port n Control 13)
...
//(15) == Advanced Control Registers (Port n Control 14) -- Enable Port Tx Queue 3 Ratio
...
//(16) == Advanced Control Registers (Port n Control 15) --Enable Port Tx Queue 2 Ratio
...
//(17) == Advanced Control Registers (Port n Control 16) --Enable Port Tx Queue 1 Ratio
...
//(18) == Advanced Control Registers (Port n Control 17) --Enable Port Tx Queue 0 Ratio
...
//(19) == Advanced Control Registers (Port n Rate Limit Control)
...
//(16) == Advanced Control Registers (Port n Queue Egress Limit Control)
...
//Testing Registers
#define REG_TESTING					0xBF //==0x80
#define REG_TESTING_2				0xDF //==0x0C
#define REG_TESTING_3				0xEF //==0x32
/* PME */
...
#define P_PHY_CTRL 				REG_PORT_CTRL_9 //0x1C YOON
