...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    {
      timeout++;
    } while (!(RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO));
...
#if (PHYCHIP == DP83848)
    RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_SR_DP83848);
...
uint32_t ETH_Prepare_Transmit_Descriptors(u16 FrameLength)
{
  ...
  __IO ETH_DMADESCTypeDef *currDMATxDesc;
