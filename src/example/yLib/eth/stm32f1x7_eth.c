...
/**
  ...
  *
  *
  * YOON :  add y811BroadR_SetupNormalMiiMode() in ETH_INIT()
  *         add ETH_Prepare_Transmit_Descriptors_TimeStamp() for PTP
  *         add several PTP functions
  * Note:   Ethernet PTP PPS control register (ETH_PTPPPSCR):
  * - Default 0000 : 1 Hz with a pulse width of 125 ms for binary rollover(when ETH_PTPTSCR's TSSSR=0). (and, of 100 ms for digital rollover when ETH_PTPTSCR's TSSSR=1)
  ******************************************************************************
  */
/* Includes ------------------------------------------------------------------*/
...
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
...
#if (SWITCH_ID == SWITCH_NOSWITCH) //7-port Switch for PTP/AVB
  ...
  ETH_InitStruct->ETH_DMAArbitration = ETH_DMAArbitration_RoundRobin_RxTx_2_1; //was 1_1
}
extern void y811BroadR_SetupNormalMiiMode();
/**
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  unsigned int result;//YOON
  uint32_t hclk = 168000000;//was 60000000; //168000000 YOON
...
#if ((PHYCHIP == DP83848) || (PHYCHIP == BCM89810) || (PHYCHIP == IP101))// || (PHYCHIP == TJA1100))
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    {
      timeout++;
    } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR, &result) & PHY_Linked_Status) && (timeout < PHY_READ_TO));

    /* Return ERROR in case of timeout */
    if(timeout == PHY_READ_TO)
    {
      err = ETH_ERROR;
      printf("> AN Failed. Go to Error\r\n");
      goto error;
    }
    ...
    {
      timeout++;
    } while (!(RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_BSR, &result) & PHY_AutoNego_Complete) && (timeout < (uint32_t)PHY_READ_TO));
...
#if (PHYCHIP == DP83848)
    RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_SR_DP83848, &result);
...
#else
    /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
    //if((RegValue & PHY_FULLDUPLEX_100M) != (uint32_t)RESET)   {
      /* Set Ethernet duplex mode to Full-duplex following the auto-negotiation */
      ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
      ETH_InitStruct->ETH_Speed = ETH_Speed_100M;
    //} else {
...
error:
  ...
  //ETH_PowerDownCmd(DISABLE); //YOON
  //ETH_MMCCounterFreezeCmd(ENABLE);
...
/**
  ...
  *     @arg ETH_MAC_FLAG_TST  : Time stamp trigger flag -------- YOON
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ETH_MAC_IT_TST   : Time stamp trigger interrupt -- PTP
...
FrameTypeDef ETH_Get_Received_Frame_byPollingMethod(void)
{
  ...
  framelength = ((DMARxDescToGet->Status & ETH_DMARxDesc_FL) >> ETH_DMARxDesc_FrameLengthShift) - 4; //If ETH_AutomaticPadCRCStrip_Enable, No -4.
...
FrameTypeDef ETH_Get_Received_Frame_interrupt(void)
{
         ...
         (descriptor_scan_counter<ETH_RXBUFNB))
  {
    ...
    else
    {
      ...
      frame.length = ((DMARxDescToGet->Status & ETH_DMARxDesc_FL) >> ETH_DMARxDesc_FrameLengthShift) - 4;//was frame.length = ((DMARxDescToGet->Status & ETH_DMARxDesc_FL) >> ETH_DMARxDesc_FrameLengthShift);//was YOON
...
uint32_t ETH_Prepare_Transmit_Descriptors(u16 FrameLength)
{
  ...
  if((DMATxDescToSet->Status & ETH_DMATxDesc_OWN) != (u32)RESET) //if it is owned by DMA, then return.
  {
  ...
  {
	  /* Set frame size */
	  currDMATxDesc->ControlBufferSize = FrameLength & ETH_DMATxDesc_TBS1;

	  /*set LAST and FIRST segment */
	  currDMATxDesc->Status |= (ETH_DMATxDesc_FS|ETH_DMATxDesc_LS);

	  /* Set Own bit of the Tx descriptor Status: gives the buffer to ETHERNET DMA */
  ...
  DMATxDescToSet = (ETH_DMADESCTypeDef *)(currDMATxDesc->Buffer2NextDescAddr); //YOON
  //DMATxDescToSet = currDMATxDesc; //actually it is the next.
...
{
  /* check if last segment --> return 1 */
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBufCount)
{
  ...
  for(i=0; i < TxBufCount; i++)
  {
    ...
    {
      /* Set next descriptor address register with next descriptor base address */
    	DMATxDesc->Buffer2NextDescAddr = (uint32_t )(DMATxDescTab+i+1); //(&DMATxDescTab[i+1]);//
    }
    else
    {
      /* For last descriptor, set next descriptor address register equal to the first descriptor base address */
      DMATxDesc->Buffer2NextDescAddr = (uint32_t) DMATxDescTab; //(uint32_t ) &DMATxDescTab[0];
    }
  }

  /* Set the start addr in Transmit Desciptor List Address Register */
...
/**
  ...
  *     @arg ETH_DMATxDesc_TTSE: Transmit Time Stamp Enable --- PTP
  ...
  *     @arg ETH_DMATxDesc_TTSS: Tx Time Stamp Status --- PTP
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ETH_DMA_FLAG_TST : Time-stamp trigger flag -- PTP
...
//See also ETH_DMARxDescReceiveITConfig( &DMARxDscrTab[i], ENABLE ); //
/**
...
/**
  ...
  *   This parameter can be one of the following values:
  *     @arg ETH_DMA_IT_TST : Time-stamp trigger interrupt --PTP
...
uint16_t ETH_ReadPHYRegister(uint16_t PHYAddress, uint16_t PHYReg,
		unsigned int *result)// //SUCCESS=1-- YOON
{
  uint32_t tmpreg = 0;
  volatile uint32_t timeout = 0;//__IO uint32_t timeout = 0;
  /* Check the parameters */
  ...
  {
	  *result = ETH_ERROR;
	  return (uint16_t)ETH_ERROR; //0
  }

  /* Return data register value */
  *result = ETH_SUCCESS; //1 -- YOON
...
uint32_t ETH_PHYLoopBackCmd(uint16_t PHYAddress, FunctionalState NewState)
{
  ...
  tmpreg = ETH_ReadPHYRegister(PHYAddress, PHY_BCR, &result);
...
void ETH_PTPUpdateMethodConfig(uint32_t UpdateMethod)
{
  ...
  {
	  /* Enable the PTP Coarse Update method *//* -----Disable the PTP Coarse Update method */
...
void ETH_PTPTimeStampCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ETH->PTPTSCR |= ETH_PTPTSCR_TSE //bit 0 : time stamp enable
    		        // | ETH_TSSEME //bit 14 : snapshot for Event msg(SYNC, DelayRQ, PdelayRQ, PdelayRESP).
    				| ETH_PTPTSCR_TSSIPV4FE //bit 13: snapshot for IPv4 frames
    				| ETH_PTPTSCR_TSSIPV6FE //bit 12: snapshot for IPv6 frames
    				//| ETH_PTPPEFE //bit 11 : snapshot for PTP over ethernet frames.
    				//| ETH_TSSSR //bit 9 : subsecond rollover (0=binary rollover; 1=digital rollover) : We use binary rollover.
    				| ETH_PTPTSCR_TSSARFE;  //bit 8 : snapshot for all received frames
    				//Other bits (interrupt, time update, init, fine/coarse) will be set other functions.
  }
  else
  {
...
//Set PPS Frequency in binary rollover. -- YOON
