...
FLASH_Status FLASH_ProgramWord(unsigned long Address, unsigned long Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    *(vu16*)Address = (unsigned short)Data;
...
{
  unsigned short WRP0_Data = 0xFFFF, WRP1_Data = 0xFFFF, WRP2_Data = 0xFFFF, WRP3_Data = 0xFFFF;
  ...
  FLASH_Pages = (unsigned long)(~FLASH_Pages);
...
FLASH_Status FLASH_UserOptionByteConfig(unsigned short OB_IWDG, unsigned short OB_STOP, unsigned short OB_STDBY)
{
  ...
  {  
    ...
    OB->USER = ( OB_IWDG | OB_STOP |OB_STDBY) | (unsigned short)0xF8;
...
unsigned long FLASH_GetUserOptionByte(void)
{
  ...
  return (unsigned long)(FLASH->OBR >> 2);
}
...
unsigned long FLASH_GetWriteProtectionOptionByte(void)
{
  ...
  return (unsigned long)(FLASH->WRPR);
}
...
FlagStatus FLASH_GetReadOutProtectionStatus(void)
{
  ...
  if ((FLASH->OBR & RDPRT_Mask) != (unsigned long)RESET)
  {
    readoutstatus = SET;
  }
  else
...
FlagStatus FLASH_GetPrefetchBufferStatus(void)
{
  ...
  if ((FLASH->ACR & ACR_PRFTBS_Mask) != (unsigned long)RESET)
  {
    bitstatus = SET;
  }
  else
...
void FLASH_ITConfig(unsigned short FLASH_IT, FunctionalState NewState)
{
  ...
  else
  {
    ...
    FLASH->CR &= ~(unsigned long)FLASH_IT;
  }
}
...
FlagStatus FLASH_GetFlagStatus(unsigned short FLASH_FLAG)
{
  ...
  {
    if((FLASH->OBR & FLASH_FLAG_OPTERR) != (unsigned long)RESET)
    {
      bitstatus = SET;
    }
    else
  ...
  {
   if((FLASH->SR & FLASH_FLAG) != (unsigned long)RESET)
    {
      bitstatus = SET;
    }
    else
